...
public class MarkertingCountDao extends BaseDao implements IMarketingCountDao
{
	...
	public DatagridVo<MarketingCount> getTodayMarketingCount(Pagination pagination,String salesmanName) {
		...
		String sql = "SELECT U.*, COUNT(DISTINCT O5.ID) MONTH_TALKING_ORDER_COUNT, COUNT(DISTINCT I2.ID) TRACING_INFOER_COUNT FROM "
				+ "("
				+ "	SELECT U.*, COUNT(DISTINCT O2.ID) TODAY_CLIENT_ADD_COUNT, COUNT(DISTINCT O3.ID) CONTACTING_CLIENT_COUNT, COUNT(DISTINCT O4.ID) DEAL_ORDER_COUNT "
				+ "	FROM"
				+ "	("
				+ "		SELECT U.ID, U.ID SALESMAN_ID, U.ALIAS SALESMAN_NAME, LU.ID LEADER_ID, COUNT(DISTINCT IV.ID) TODAY_INFOER_VISIT_COUNT, "
				+ "			COUNT(DISTINCT OV.ID) TODAY_ORDER_VISIT_COUNT, COUNT(DISTINCT OV1.ID) TODAY_CLIENT_VISIT_COUNT, "
				+ "			COUNT(DISTINCT O.ID) TALKING_ORDER_COUNT, COUNT(DISTINCT I.ID) TODAY_INFOER_ADD_COUNT "
				+ "		FROM USER U "
				...
				+ "		LEFT JOIN ORDER_VISIT OV FORCE INDEX(IDX_ORDER_VISIT_ORDER_STATUS_DATE, IDX_ORDER_VISIT_DATE) ON OV.ORDER_STATUS =34 AND OV.VISITOR_ID = U.ID AND OV.DATE BETWEEN CONCAT(CURRENT_DATE,' 00:00:00') AND CONCAT(CURRENT_DATE,' 23:59:59') "
				+ "		LEFT JOIN ORDER_VISIT OV1 FORCE INDEX(IDX_ORDER_VISIT_ORDER_STATUS_DATE, IDX_ORDER_VISIT_DATE) ON OV1.ORDER_STATUS IN(10,12,14,20,22,30,32) AND OV1.VISITOR_ID = U.ID AND OV1.DATE BETWEEN CONCAT(CURRENT_DATE,' 00:00:00') AND CONCAT(CURRENT_DATE,' 23:59:59') "
		...
		if(StringUtils.isNotEmpty(salesmanName))
		{
			...
			sql+= "		GROUP BY U.ID, LU.ID "
				...
				+ "	GROUP BY U.ID, U.LEADER_ID "
				+ ") U "
				+ "LEFT JOIN `ORDER` O5 FORCE INDEX(IDX_ORDER_STATUS) ON U.ID = O5.SALESMAN_ID AND O5.`STATUS` IN(20,22,34,90,0,60,62,64) AND O5.INSERT_TIME BETWEEN CONCAT(DATE_FORMAT(CURRENT_DATE,'%Y-%m'),'-01 00:00:00') AND CONCAT(LAST_DAY(CURRENT_DATE),' 23:59:59') "
				+ "LEFT JOIN INFOER I2 ON I2.SALESMAN_ID = U.ID "
				+ "WHERE 1 = 1 "
				+ "GROUP BY U.ID, U.LEADER_ID "
				+ "ORDER BY U.SALESMAN_NAME ";
